    ...
    return (
      <DashboardLayout>
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
          <div className="text-center bg-white p-8 sm:p-12 rounded-lg shadow-lg max-w-md w-full">
            <div className="text-6xl mb-4">✅</div>
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen bg-gray-50 pb-24 lg:pb-0">
        ...
        <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              <div className={`flex-1 h-1.5 sm:h-2 rounded ${step >= 1 ? 'bg-orange-500' : 'bg-gray-200'}`}></div>
              <div className={`flex-1 h-1.5 sm:h-2 rounded ${step >= 2 ? 'bg-orange-500' : 'bg-gray-200'}`}></div>
              <div className={`flex-1 h-1.5 sm:h-2 rounded ${step >= 3 ? 'bg-orange-500' : 'bg-gray-200'}`}></div>
            </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="lg:hidden bg-white rounded-lg shadow-sm overflow-hidden">
                <button
                  ...
                  className="w-full px-4 py-3 flex items-center justify-between bg-gray-50 text-sm font-medium text-gray-900"
                >
              ...
              {step === 1 && (
                <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                    ...
                    {cart.map((item) => (
                      <div key={`${item.product._id}-${item.size}`} className="flex gap-4 pb-4 border-b last:border-b-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="font-medium text-gray-900 mb-1 truncate">{item.product.title}</h3>
                          <p className="text-sm text-gray-600 mb-2">Size: {item.size}</p>
                          <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                            <div className="flex items-center border border-gray-300 rounded">
                              <button
                                onClick={() => updateQuantity(item.product._id, item.size, item.quantity - 1)}
                                className="px-2 py-1 sm:px-3 hover:bg-gray-100"
                              >
                                −
                              </button>
                              <span className="px-2 py-1 sm:px-3 font-medium text-sm">{item.quantity}</span>
                              <button
                                onClick={() => updateQuantity(item.product._id, item.size, item.quantity + 1)}
                                className="px-2 py-1 sm:px-3 hover:bg-gray-100"
                              >
                  ...
                  <button
                    ...
                    className="hidden lg:block w-full mt-6 px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition-colors"
                  >
                    Proceed to Address
                  </button>
                </div>
                  ...
                  <form id="address-form" onSubmit={handleAddressSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <input
                        ...
                        className="px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-base"
                        required
                      />
                      <input
                        type="tel"
                    ...
                    <button
                      type="submit"
                      className="hidden lg:block w-full px-6 py-3 bg-orange-500 hover:bg-orange-600 text-white font-semibold rounded-lg transition-colors"
                    >
                      Proceed to Payment
                    </button>
                  </form>
                ...
                <div className="space-y-6">
                  <div className="bg-white rounded-lg shadow-sm p-4 sm:p-6">
                    ...
                    <button
                      ...
                      className="hidden lg:block w-full mt-6 px-6 py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors"
                    >
                      Place Order
                    </button>
                  </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 lg:hidden z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
          <div className="flex items-center justify-between gap-4">
            ...
            {step === 2 && (
              <button
                onClick={() => document.getElementById('address-form').requestSubmit()}
                className="flex-1 bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-lg transition-colors"
              >
                Proceed to Payment
              </button>
            ...
            {step === 3 && (
              <button
                onClick={handlePlaceOrder}
                disabled={!selectedPaymentMethod || (selectedPaymentMethod !== 'cod' && !selectedPaymentOption)}
                className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-lg transition-colors"
              >
                Place Order
              </button>
            )}
