      ...
      <div className="min-h-screen bg-gray-50 pb-24 lg:pb-0">
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="px-4 py-3 sm:px-6 sm:py-4">
            ...
            <div className="text-xs sm:text-sm text-gray-600 overflow-x-auto whitespace-nowrap pb-1 scrollbar-hide">
              <Link to="/products" className="hover:text-gray-900">Store</Link>
              {' > '}
              <span>Fitzdo & Sports</span>
              {' > '}
              <span className="hidden sm:inline">Exercise & Fitness</span>
              <span className="hidden sm:inline">{' > '}</span>
              <span className="hidden sm:inline">Cardio & Yoga</span>
              <span className="hidden sm:inline">{' > '}</span>
              ...
              <div>
                <div className="relative mb-4 bg-gray-50 rounded-lg p-6">
                  ...
                  {/* {product.tags?.includes('Fitzdo Sponsored') && (
                    <span className="absolute top-4 left-4 sm:top-8 sm:left-8 bg-gray-100 text-gray-700 text-xs font-medium px-3 py-1.5 rounded">
                      Fitzdo Sponsored
                    </span>
                  )} */}
                </div>
                ...
                <div className="space-y-2">
                  {/* What's in the Box */}
                  <div className="border mt-2 border-gray-300 rounded-lg overflow-hidden">
                    <button
              ...
              <div>
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <span
                          key={i}
                          className={`text-lg sm:text-xl ${i < Math.floor(product.rating) ? 'text-yellow-400' : 'text-gray-300'}`}
                        >
                          ★
                        </span>
                      ))}
                ...
                <div className="mb-6 pb-6 border-b">
                  ...
                  <div className="space-y-3">
                    {bankOffers.slice(0, showAllOffers ? bankOffers.length : 1).map((offer) => (
                      <div key={offer.id} className="bg-red-50 border-l-4 border-red-500 p-4 rounded">
                        <div className="flex items-start gap-3">
                          <div className="bg-red-500 text-white px-3 py-2 rounded font-bold text-xs shrink-0">
                            OFFER
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-600 mb-1 line-clamp-2">{offer.description}</p>
                    ...
                    {showAllOffers && cashbackOffers.map((offer) => (
                    ...
                    >
                      {showAllOffers ? 'See Less' : `See ${bankOffers.length + cashbackOffers.length - 1} More Offers`}
                      <svg className={`w-4 h-4 transform transition-transform ${showAllOffers ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 lg:hidden z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
          {showAddedToCart && (
            <div className="absolute bottom-full left-0 right-0 bg-green-500 text-white px-4 py-2 text-center text-sm font-medium animate-fade-in-up">
              ✅ Added to cart!
            </div>
          )}
          <div className="flex gap-3">
            ...
            <button
              ...
              className="flex-1 bg-black hover:bg-gray-800 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-lg transition-colors"
            >
              Buy Now
            </button>
          </div>
